...
def main(args):
    ...
    for idx, image_file in enumerate(image_file_list):
        ...
        for index, img in enumerate(imgs):
            ...
            res = [{
                "transcription": re.sub(r"[^\uAC00-\uD7A30-9a-vx-zA-VX-Z\s]", "", rec_res[i][0]),
                "points": np.array(dt_boxes[i]).astype(np.int32).tolist(),
            } for i in range(len(dt_boxes)) if re.sub(r"[^\uAC00-\uD7A30-9a-vx-zA-VX-Z\s]", "", rec_res[i][0]).isdigit()]
            aa = [r['transcription'] for r in res]
            print(aa)
            if len(imgs) > 1:
            ...
            if is_visualize:
                ...
                boxes = dt_boxes
                txts = [rec_res[i][0] for i in range(len(rec_res)) if re.sub(r"[^\uAC00-\uD7A30-9a-vx-zA-VX-Z\s]", "", rec_res[i][0]).isdigit()] 
                scores = [rec_res[i][1] for i in range(len(rec_res)) if re.sub(r"[^\uAC00-\uD7A30-9a-vx-zA-VX-Z\s]", "", rec_res[i][0]).isdigit()]
